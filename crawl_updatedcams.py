...
#    main(base_url,cameras13,camloc13,start);
    global timer 
    timer = threading.Timer(0.001,crawl_timer)
    timer.start() 
    time.sleep(2)
#    print(time.time())
    timer.cancel()
...
cameras = []
cameras.append([846,165,551,366,472]) # 2nd Ave 
cameras.append([845,403,398,410,399]) # 3rd Ave
...
#    for i in range(len(cameras[n])):
    camloc = mkdir_camloc(base_url, cameras[n])
    camlocs.append(camloc)
